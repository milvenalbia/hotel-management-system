<div>
<div wire:ignore.self class="modal fade" id="editCheckInModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
                    ...
                    <div class="cotainer ml-3">
                        <form wire:submit.prevent="submit">
                            ...
                            <div class="d-flex justify-content-between mb-3 form-group">
                                <div class="form-group w-100 mr-3">
                                    <label for="extend_hours">Extend Hour (1 Hour Only)</label>
                                    <div class="input-group">
                                        <button wire:click="decrement('hours')" type="button" class="btn rounded-0" style="background: #2A3038;"><i class="mdi mdi-minus"></i></button>
                                            <input wire:model.live.debounce.500ms="extend_hours" class="form-control text-center" id="extend_hours" type="number"  min="0" placeholder="No. of Extend Hours" readonly/>
                                        <button wire:click="increment('hours')" type="button" class="btn rounded-0" style="background: #2A3038;"><i class="mdi mdi-plus"></i></button>
                                    </div>
                                    @error('extend_hours')
                                        <span class="text-danger text-xs">{{$message}}</span> 
                                    @enderror
                                ...
                                <div class="form-group w-100">
                                    <label for="extend_days">Extend Days</label>
                                    <div class="input-group">
                                            <button wire:click="decrement('days')" type="button" class="btn rounded-0" style="background: #2A3038;"><i class="mdi mdi-minus"></i></button>
                                                <input wire:model.live.debounce.500ms="extend_days" class="form-control text-center" id="extend_days" type="number"  min="0" placeholder="No. of Extend Days" readonly/>
                                            <button wire:click="increment('days')" type="button" class="btn rounded-0" style="background: #2A3038;"><i class="mdi mdi-plus"></i></button>
                                        </div>
                                    @error('extend_days')
                                        <span class="text-danger text-xs">{{$message}}</span> 
                                    @enderror
                        ...
                        <table class="table mt-3">
                            <tbody>
                                <tr>
                                    <td>Added Cost (Extend Hours & Days):</td>
                                    <td>&#8369;{{number_format($extended_amount, 2, '.', ',')}}</td>
                                </tr>
                                <tr>
                        ...
                        {{-- @if($checked_button == true || $roomtype_id != $roomtype_fetch_id)
                            @if($children_no > 0 && $room_no == '')
                                ...
                                {{ $errors->any() || is_null($firstname) || is_null($lastname) || is_null($check_in) || is_null($check_out) || is_null($roomtype_id) || is_null($room_id) || in_array('', $child_ages) ? 'disabled' : '' }}>Update Check In</button>
                            @elseif($children_no == 0 && $room_no > 0)
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill w-100 mt-4"
                                {{ $errors->any() || is_null($firstname) || is_null($lastname) || is_null($check_in) || is_null($check_out) || is_null($roomtype_id) || in_array('', $room_id) ? 'disabled' : '' }}>Update Check In</button>
                            @elseif($children_no > 0 && $room_no > 0)
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill w-100 mt-4"
                                {{ $errors->any() || is_null($firstname) || is_null($lastname) || is_null($check_in) || is_null($check_out) || is_null($roomtype_id) || in_array('', $room_id)|| in_array('', $child_ages) ? 'disabled' : '' }}>Update Check In</button>
                            @else
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill w-100 mt-4"
                                {{ $errors->any() || is_null($firstname) || is_null($lastname) || is_null($check_in) || is_null($check_out) || is_null($roomtype_id) || is_null($room_no) || is_null($adult_no) || $adult_no == 0 || $room_no == 0 ? 'disabled' : '' }}
                        ...
                        @else
                            ...
                            @else
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill w-100 mt-4"
                                {{ $errors->any() || is_null($firstname) || is_null($lastname) || is_null($check_in) || is_null($check_out) || is_null($roomtype_id) ? 'disabled' : '' }}
                                >Update Check In</button>
                            @endif
                        @endif --}}
...
<div wire:ignore.self class="modal fade" id="conflictModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content border-light" style="background-color: #ffffff;">
            ...
            <div class="modal-body text-center text-dark">
                Apologies, but the extended stay you requested for this room has been booked by another guest!
            </div>
            <div class="modal-footer">
                <button class="btn btn-primary py-2 px-4" wire:click="closeConflictModal">Ok</button>
            </div>
        </div>
    </div>
</div>
...
<div wire:ignore.self class="modal fade" id="infoModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content border-light" style="background-color: #ffffff;">
            <div class="modal-header border-light">
                <h5 class="modal-title text-dark" id="infoModalLabel">Notification</h5>
                </button>
            </div>
            <div class="modal-body text-center text-dark">
            ...
            <div class="modal-footer">
                <button class="btn btn-md btn-primary py-2 px-4" wire:click="closeInfoModal">Ok</button>
            </div>
        </div>
    </div>
</div>
